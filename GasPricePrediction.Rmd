...
###Model-1: Linear Regression Model considering stocks, no of days of supply, refinary capacity, export data and crude price
```{r lm1}
...
### Model-7: Linear Regression Model considering all the independent variables  
```{r lm7}
gasprice_lm7 <- lm(Gas_price_per_gallon ~ Crude_price_per_barrel	+ Stocks_per_thousand_barrels +	No_of_days	+ Utilization_percentage +	Exp_thousand_barrels_per_day + Imp_thousand_barrels_per_day, data = gas_price_train)
...
rank(maes)
```
So, `lm7` has the lowest MAE followed by `lm1` and then `lm8`.
...
#### Constant variance
```{r constant_variance}
ggplot(data.frame(residuals=gasprice_lm7$residuals,
                  fitted=gasprice_lm7$fitted.values)) + geom_point(aes(x=fitted, y=residuals))
```
...
price_predict <- data.frame(predict7=predict(gasprice_lm7, newdata=gas_price_test),
                             predict1=predict(gasprice_lm1, newdata=gas_price_test),
...
price_predict_MAE <- data.frame(predict7_MAE=MAE(price_predict$predict7, gas_price_test$Gas_price_per_gallon),
                                predict1_MAE=MAE(price_predict$predict1, gas_price_test$Gas_price_per_gallon),
...
```{r}
finalpredtestData<-data.frame(gas_price_test$Date,price_predict$predict7)
colnames(finalpredtestData) <-c("Date","GasPrice")
```

```{r}
ggplot(finalpredtestData,aes(Date,GasPrice))+ geom_line() +
xlab("Date")+ ylab("Gas Price ($)") + 
  scale_x_date(limits=as.Date(c('2021-01-01','2022-03-01'), breaks=date_breaks(width="6 months")))
```
